...
`tmfast` addresses these problems directly. Its fitting algorithm is deterministic — results are fully reproducible without setting a random seed — and requires no specification of Bayesian priors. In tests with simulated data, `tmfast` fits topic models approximately 20× faster than `stm` [@RobertsStmPackageStructural2019], making it practical to fit and compare many models as part of a routine analysis workflow.
...
The topic modeling approach used in `tmfast` does have three notable limitations compared to `stm`. First, there is no way to incorporate document-level metadata into the models. Second, scores and loadings for varimax-rotated partial PCA are not directly interpretable as probability distributions. Casting them into distributions introduces a notable researcher degree of freedom, comparable to the choice of Bayesian priors of LDA. Third, in tests with simulated data, `tmfast` was somewhat less accurate than `stm` at recovering true topic-document and word-topic distributions. 

# Software Design

## Fitting pipeline

`tmfast` fits topic models in two steps. First, `irlba::prcomp_irlba()` is used to compute a truncated PCA of the document-term matrix, retaining only the top principal components needed for the largest requested number of topics ($k$). Because PCA is computed only once, multiple models with different values of $k$ can be extracted from a single fit, applying varimax rotation at each requested $k$ without repeating the—relatively computationally intensive—truncated PCA step.
...
`ndH()` measures the information gain a term carries beyond what would be expected if it were distributed uniformly across documents. More precisely, it involves $\Delta H = \log_2(D) - H(p)$, equivalent to KL divergence from the uniform distribution, where $D$ is the number of documents and $H(p)$ is the observed entropy of the term's document distribution. $\Delta H$ is then weighted by logged term frequency $\log_2(n)$ to down-weight rare terms that may be OCR artifacts or misspellings: $\mathit{ndH} = \log_2(n) \cdot \Delta H$.

`ndR()` uses information gain/KL divergence relative to a length-proportional document distribution rather than the uniform distribution: $p(d) = l_d / sum_d(l_d)$ where $l_d$ is the word length of document $d$, so that short documents (such as short articles) are less probable than long documents (such as books). We then calculate KL divergence for the word from this length-proportional distribution: $\Delta R = \sum_d p(d \mid w) \log_2 [p(d \mid w) / p(d)]$. Because short documents are less probable, terms that are concentrated to short documents are "surprising" (high information gain) and thus tend to have a greater $\Delta R$, making this approach preferable when document lengths vary significantly. We then weight by $\log_2(n)$ as with `ndH`. 
...
`journal_specific()` provides a complete benchmarking simulation: it generates a corpus with known true topic and word distributions ("specific" to each simulated "journal"), fits `tmfast`, aligns fitted topics to true topics as a linear assignment problem, and returns recovery accuracy as mean Hellinger distances for both $\phi$ (true word-topic) and $\theta$ (true topic-document) distributions. This enables systematic evaluation of how recovery accuracy scales with corpus size, vocabulary size, and number of topics.

# Research Impact Statement

`tmfast` was originally developed to support the text analysis in @HicksRaceScienceMainstream2025, which used topic models to study race science in mainstream psychology journals from 1960 to 2010. `tmfast` has subsequently been developed as a standalone package for the broader text mining community. The package has an arXiv preprint (arXiv:2305.01535, posted May 2023) and was first released on CRAN (version 0.1.1) in May 2026. 

# AI Usage Disclosure

Claude Sonnet 4.6, via Claude Code, was used to prepare this paper. Claude was also used to prepare `tmfast` for CRAN submission, especially writing unit tests and documentation. Prior to March 2026 no generative AI was used to develop the package. I (the human author) confirm that I reviewed, edited, and validated all AI-assisted outputs and made the core design decisions. 
