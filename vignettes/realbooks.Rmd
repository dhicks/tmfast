...
We analyze works by 10 authors of the long nineteenth century: Jane Austen, Anne, Charlotte, and Emily Brontë, Louisa May Alcott, George Eliot, Mary Shelley, Charles Dickens, HG Wells, and HP Lovecraft.  The corpus is distributed as a pre-downloaded snapshot via the [`tmfast.realbooks`](https://github.com/dhicks/tmfast.realbooks) package (install with `install.packages("tmfast.realbooks")`).  We load it and use `tidytext::unnest_tokens()` to convert it into a long-format document-term matrix.
...
To reproduce the corpus download from scratch (requires network access and several minutes), see `data-raw/download.R` in the `tmfast.realbooks` package.
