...
The corpus is provided in the [`tmfast.realbooks` data package](https://github.com/dhicks/tmfast.realbooks), which is served by the package repository <https://dhicks.github.io/drat/>. In turn, this repository is made available when `tmfast` is loaded: 
```
#| eval: false
library(tmfast)
install.packages('tmfast.realbooks')
```
